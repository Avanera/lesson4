class Train
  ...
  def add_wagons(wagon)
    puts "The wagons quantity of train #{self.number} is #{@wagons_quant.length}."
  end

  def delete_wagons(wagon)
    @wagons_quant.delete(wagon) if @speed == 0 && @wagons_quant.length >= 1  
    puts "The wagons quantity of train #{self.number} is #{@wagons_quant.length}."
  end

=begin
  def show_wagons_quant
    puts "The wagons quantity of train #{self.number} is #{@wagons_quant}."
  end

  def add_wagons
    @wagons_quant += 1 if @speed == 0
    puts "The wagons quantity of train #{self.number} is #{@wagons_quant}."
  end

  def delete_wagons
    @wagons_quant += @wagons_quant - 1 if @speed == 0 && @wagons_quant >= 1
    puts "The wagons quantity of train #{self.number} is #{@wagons_quant}."
  end
=end
